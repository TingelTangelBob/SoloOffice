  ...
  return <div className="space-y-6">
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-3 sm:gap-4">
        <article className="rounded-xl border border-gray-200 p-4 lg:p-5"><div className="flex items-center gap-3"><ReceiptText className="h-5 w-5 text-primary-custom" /><span className="text-sm text-gray-500">Bezahlte Rechnungen</span></div><p className="mt-3 text-2xl font-bold text-gray-900">{paidInvoiceSummary.count}</p><button type="button" onClick={() => onNavigate('invoices')} className="action-button mt-4">Rechnungen prüfen <ArrowRight className="h-4 w-4" /></button></article>
        <article className="rounded-xl border border-gray-200 p-4 lg:p-5"><div className="flex items-center gap-3"><FileSpreadsheet className="h-5 w-5 text-emerald-600" /><span className="text-sm text-gray-500">Automatische Einnahmen</span></div><p className="mt-3 text-2xl font-bold text-emerald-700">{formatAmount(paidInvoiceSummary.income)}</p><button type="button" onClick={() => onNavigate('euer')} className="action-button mt-4">EÜR-Vorschau öffnen <ArrowRight className="h-4 w-4" /></button></article>
        <article className="rounded-xl border border-gray-200 p-4 lg:p-5"><div className="flex items-center gap-3"><CheckCircle2 className="h-5 w-5 text-amber-600" /><span className="text-sm text-gray-500">Nächster Schritt</span></div><p className="mt-3 font-semibold text-gray-900">Ausgaben ergänzen</p><button type="button" onClick={() => onNavigate('euer')} className="action-button mt-4">Ausgaben erfassen <ArrowRight className="h-4 w-4" /></button></article>
      </div>
    </section>
