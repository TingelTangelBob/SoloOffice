...
function dateInputValuePlusDays(dateValue: string, days: number): string {
...
export function JobInvoiceGenerator({ 
  ...
  const [issueDateOverride, setIssueDateOverride] = useState(() => dateInputValue(new Date()));
  const [dueDateOverride, setDueDateOverride] = useState(() => dateInputValuePlusDays(dateInputValue(new Date()), company?.defaultPaymentDays ?? 30));
  ...
  const handleGenerate = () => {
    ...
    const invoiceBatchCount = getInvoiceBatchCount();
    if (!issueDateOverride || !dueDateOverride) {
      notify({ variant: 'warning', message: 'Bitte Rechnungsdatum und Fälligkeitsdatum angeben.' });
      return;
    }
    if (manualInvoiceNumber.trim() && invoiceBatchCount !== 1) {
      notify({ variant: 'warning', message: 'Eine eigene Rechnungsnummer ist nur möglich, wenn genau eine Rechnung erstellt wird. Bei Sammelläufen wird der Nummernkreis verwendet.' });
      return;
    }
  ...
  const createInvoiceForJobs = async (jobsToInvoice: JobEntry[]) => {
    ...
    const issueDate = new Date(`${issueDateOverride}T00:00:00`);
    ...
    const dueDate = new Date(`${dueDateOverride}T00:00:00`);
    const invoiceBatchCount = getInvoiceBatchCount();
    
    const invoice = {
      invoiceNumber: invoiceBatchCount === 1 ? manualInvoiceNumber.trim() : '',
    ...
    <div className="dialog-overlay fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-4 space-y-6">
          ...
          {/* Rechnungskopf für Rück- und Sammelrechnungen */}
          <div>
            <h4 className="mb-3 text-sm font-medium text-gray-900">Rechnungsangaben</h4>
            <div className="grid grid-cols-1 gap-3 tablet:grid-cols-2">
              <label className="block text-sm font-medium text-gray-700">
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    setIssueDateOverride(nextIssueDate);
                    if (nextIssueDate) setDueDateOverride(dateInputValuePlusDays(nextIssueDate, company?.defaultPaymentDays ?? 30));
                  }}
              ...
              <label className="block text-sm font-medium text-gray-700 tablet:col-span-2">
                Eigene Rechnungsnummer
                <input
                  type="text"
                  value={manualInvoiceNumber}
                  onChange={(event) => setManualInvoiceNumber(event.target.value)}
                  disabled={getInvoiceBatchCount() !== 1}
                  placeholder={getInvoiceBatchCount() === 1 ? 'Leer = automatische Vergabe' : 'Nur bei genau einer Rechnung möglich'}
                  className="form-input form-input-compact mt-1 w-full disabled:bg-gray-100 disabled:text-gray-500"
                />
              </label>
            </div>
            <p className="mt-2 text-xs text-gray-500">
              Standardmäßig werden Rechnungsdatum heute und die Fälligkeit nach den Zahlungseinstellungen verwendet. Für mehrere Rechnungen gilt die Datumsangabe gemeinsam; die eigene Nummer ist dort aus Gründen der Eindeutigkeit deaktiviert.
            </p>
          </div>
